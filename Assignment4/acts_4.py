...
def check_user(username):
    if(crash_flag==1):
		return jsonify({}),500
    users_all = requests.get("http://172.31.32.90:80/api/v1/users")
...
def totalActsCount():
    ...
    try:
        actsCount = act.count_documents({})
        return jsonify([actsCount]), 200
    except:
        actsCount = 0
        return jsonify([actsCount]), 200
...
def healthCheck():
	...
	#Checking for working of GET APIs

	#List users
	check = requests.get("http://172.31.32.90:80/api/v1/users")
	if(check.status_code!=200 and check.status_code!=204):
		return jsonify({}),500

	#List categories
	check = requests.get("http://172.31.32.90:80/api/v1/categories")
	if(check.status_code!=200 and check.status_code!=204):
		return jsonify({}),500

	#List acts and number of acts in each category
	categ = mongo.db.categories    
	for name in categ:
			
		check = requests.get("http://172.31.32.90:80/api/v1/categories"+name+"/acts")
		if(check.status_code!=200 and check.status_code!=204):
			return jsonify({}),500

		check = requests.get("http://172.31.32.90:80/api/v1/categories"+name+"/acts/size")
		if(check.status_code!=200 and check.status_code!=204):
			return jsonify({}),500
		
	#List acts count
	check = requests.get("http://172.31.32.90:80/api/v1/acts/count")
	if(check.status_code!=200 and check.status_code!=204):
		return jsonify({}),500
